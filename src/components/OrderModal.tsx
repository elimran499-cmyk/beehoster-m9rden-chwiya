...
export const OrderModal: React.FC<OrderModalProps> = ({ isOpen, initialPlanId = 'plan-12m', onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 overflow-y-auto bg-ink/45 backdrop-blur-md animate-in fade-in duration-200">

      {/* Modal Container */}
      <div className="relative w-full max-w-lg glass-panel-accent rounded-3xl overflow-hidden my-8">

        {/* Header Bar */}
        <div className="px-5 sm:px-6 py-4 bg-surface border-b border-line flex items-center justify-between gap-3">
          ...
          <button
            onClick={onClose}
            className="p-1.5 rounded-full bg-paper border border-line text-muted hover:text-ink hover:bg-paper transition-colors shrink-0"
        ...
        <div className="p-5 sm:p-7 max-h-[80vh] overflow-y-auto space-y-6">
          ...
          {/* Step 1: Plan */}
          <div>
            <label className="text-xs font-bold text-accent-deep uppercase tracking-wider block mb-2.5">
              Welk abonnement?
            </label>
              ...
              {PRICING_PLANS.map((plan) => (
                ...
                >
                  <div className="flex items-center justify-between gap-2 mb-1">
                    ...
                    {plan.bestValue && (
                      <span className="text-[9px] font-extrabold px-1.5 py-0.5 rounded bg-accent text-white">
                        BEST
                      </span>
                    )}
                  </div>
                  <span className="text-sm font-black text-accent-deep">€{plan.price.toFixed(2)}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="p-4 rounded-2xl glass-card flex items-center justify-between gap-3">
            <div className="min-w-0">
              <span className="text-[10px] text-muted uppercase tracking-wider block">Onze WhatsApp-lijn</span>
              <span className="text-base font-bold text-ink tracking-wide">{WHATSAPP_DISPLAY}</span>
            </div>
            <button
              onClick={handleCopyNumber}
              className="shrink-0 px-3.5 py-2 text-xs font-bold text-ink border border-line rounded-xl hover:text-ink hover:bg-paper transition-colors flex items-center gap-1.5"
            >
          ...
          {/* Reassurance */}
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 text-[11px] text-muted">
            <span className="flex items-center gap-2">
              <Zap className="w-4 h-4 text-accent shrink-0" /> Actief in 5 minuten
            </span>
            <span className="flex items-center gap-2">
              <Clock className="w-4 h-4 text-accent-deep shrink-0" /> 24/7 antwoord
            </span>
            <span className="flex items-center gap-2">
              <ShieldCheck className="w-4 h-4 text-accent shrink-0" /> 7 dagen geld terug
            </span>
          </div>
