...
export const DeviceBanner: React.FC<DeviceBannerProps> = ({ onOpenOrderModal }) => {
  ...
  return (
    <section id="devices" className="relative z-10 py-16 border-y border-white/5 overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        
        {/* Section Header */}
        <div className="text-center max-w-3xl mx-auto mb-12">
          <span className="text-xs font-bold text-[#FFD166] uppercase tracking-widest bg-[#FF5C3A]/10 px-3 py-1 rounded-full border border-[#FF5C3A]/30">
            WERKT OP ALLES
          </span>
          <h2 className="mt-4 text-3xl sm:text-4xl font-extrabold text-white tracking-tight">
            Feilloos op <span className="text-[#FFD166]">al je apparaten</span>
          </h2>
            ...
            return (
              <button
                ...
                className={`p-4 rounded-2xl flex flex-col items-center text-center transition-all duration-300 relative ${
                  isSelected
                    ? 'glass-panel-accent text-white translate-y-[-2px]'
                    : 'glass-card text-zinc-400 hover:text-white'
                }`}
              >
                {device.badge && (
                  <span className="absolute -top-2 px-2 py-0.5 rounded-full text-[9px] font-bold bg-[#FF5C3A] text-black shadow-md">
                    {device.badge}
                  </span>
                )}
                
                <div className={`p-3 rounded-xl mb-3 ${isSelected ? 'bg-[#FF5C3A] text-black' : 'bg-zinc-800 text-zinc-300'}`}>
                  {getDeviceIcon(device.icon)}
                </div>

                <span className="text-xs sm:text-sm font-bold tracking-tight text-white mb-1">
                  {device.name}
                </span>
        ...
        <div className="relative glass-card glass-edge rounded-3xl p-6 sm:p-8">
          <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-center">
            
            <div className="md:col-span-7 space-y-4">
              <div className="flex items-center gap-3">
                <div className="p-3 rounded-xl bg-[#FF5C3A]/20 text-[#FFD166] border border-[#FF5C3A]/40">
                  {getDeviceIcon(selectedDevice.icon)}
                </div>
                <div>
                  <h3 className="text-xl sm:text-2xl font-bold text-white">{selectedDevice.name}</h3>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedDevice.recommendedApps.map((app) => (
                    <span
                      key={app}
                      className="px-3 py-1.5 rounded-xl bg-zinc-800 text-xs font-semibold text-zinc-200 border border-white/10 flex items-center gap-1.5"
                    >
            ...
            <div className="md:col-span-5 flex flex-col items-center justify-center p-6 rounded-2xl bg-zinc-950/80 border border-white/10 text-center">
              <div className="w-12 h-12 rounded-full bg-[#FF5C3A]/10 flex items-center justify-center text-[#FFD166] mb-3">
                <Download className="w-6 h-6 text-[#FF5C3A]" />
              </div>
              <h4 className="text-base font-bold text-white">Hulp nodig bij de installatie?</h4>
              ...
              <div className="flex flex-col sm:flex-row items-center gap-2 w-full">
                <a
                  ...
                  className="w-full py-2.5 text-xs font-semibold text-zinc-100 glass-card hover:border-white/30 rounded-xl transition-all flex items-center justify-center gap-1.5"
                >
                ...
                <button
                  ...
                  className="w-full py-2.5 text-xs font-bold text-black accent-button-gradient rounded-xl shadow-md"
                >
                  Nu starten
                </button>
              </div>
