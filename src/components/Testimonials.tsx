      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-14">
          <div className="relative inline-flex items-center gap-2 px-3 py-1 rounded-full glass-card mb-4">
            <div className="flex text-accent-deep">
              {[...Array(5)].map((_, i) => (
                <Star key={i} className="w-3.5 h-3.5 fill-accent-hi" />
              ))}
            </div>
            <span className="text-xs font-bold text-ink">4,9 / 5,0 sterren</span>
          </div>
          
          <h2 className="font-display text-4xl sm:text-5xl md:text-6xl text-ink leading-[1.06]">
                ...
                <div className="flex items-center justify-between mb-4">
                  <div className="flex text-accent-deep">
                    {[...Array(review.rating)].map((_, i) => (
                      <Star key={i} className="w-4 h-4 fill-accent-hi" />
                    ))}
                  </div>
              ...
              <div className="pt-4 border-t border-line flex items-center justify-between">
                <div>
                  <h4 className="text-sm font-bold text-ink flex items-center gap-1.5">
                    {review.name}
                    {review.verified && (
                      <CheckCircle2 className="w-3.5 h-3.5 text-accent" title="Geverifieerde klant" />
                    )}
                  </h4>
                  <span className="text-[11px] text-muted">{review.location}</span>
                </div>

                <span className="text-[10px] font-bold text-accent bg-accent-soft px-2 py-1 rounded-lg border border-accent/35">
        ...
        {/* Trustpilot Banner */}
        <div className="mt-12 text-center flex flex-col sm:flex-row items-center justify-center gap-3 text-xs text-muted">
